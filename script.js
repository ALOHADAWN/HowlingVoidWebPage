...
================================================`,
        ...
        discordPage: {
            ...
            warning: '>> WARNING: YOU ARE LEAVING THE HOWLING VOID SECURED ENVIRONMENT.',
...
function switchLanguage(lang) {
    ...
    typeHeader(translations[currentLang].systemTitle, 20, () => {
        ...
        if (area && area.innerHTML.trim() !== '') {
            ...
            if (area.innerHTML.includes('ПРИКАЗ') || area.innerHTML.includes('DIRECTIVE')) {
                showPromises();
            } else if (area.innerHTML.includes('ОТЧЕТ') || area.innerHTML.includes('REPORT')) {
                showComplianceReport();
            } else if (area.innerHTML.includes('DISCORD') || area.innerHTML.includes('ВНЕШНИЙ')) {
                showDiscordPage();
            }
        }
        
        systemMessage(lang === 'ru' 
...
function executeLanguageSwitch(lang) {
    ...
    const translateMsg = currentLang === 'ru' 
    ...
    setTimeout(() => {
        ...
        typeHeader(translations[currentLang].systemTitle, 20, () => {
            ...
            const completeMsg = currentLang === 'ru' 
                ? '>> ПЕРЕВОД ЗАВЕРШЕН. ИНТЕРФЕЙС АКТИВЕН.' 
                : '>> TRANSLATION COMPLETE. INTERFACE ACTIVE.';
            
            systemMessage(completeMsg, false);
            
            console.log(`>> LANGUAGE SWITCH COMPLETE: ${oldLang.toUpperCase()} → ${lang.toUpperCase()}`);
        });
...
function toggleSound() {
    ...
    systemMessage(currentLang === 'ru' 
        ? (soundEnabled ? '>> АУДИОСИСТЕМА: АКТИВИРОВАНА' : '>> АУДИОСИСТЕМА: ОТКЛЮЧЕНА')
        : (soundEnabled ? '>> AUDIO SYSTEM: ENABLED' : '>> AUDIO SYSTEM: DISABLED'));
}
...
function processSelection(selection) {
    ...
    if (responses[selection]) {
        ...
        setTimeout(() => {
            switch(selection) {
                case '1': showPromises(); break;
                case '2': showComplianceReport(); break;
                case '3': reevaluateTrust(); break;
                case '4': showWikiPage(); break;
                case '5': exportDossier(); break;
                case '6': showDiscordPage(); break;
                case '9': initiatePurgeProtocol(); break;
            }
        }, 800);
    } else {
...
function showDiscordPage() {
    ...
    const texts = translations[currentLang].discordPage || {
        title: currentLang === 'ru' ? 'ВНЕШНИЙ КАНАЛ СВЯЗИ' : 'EXTERNAL COMMUNICATION CHANNEL',
        warning: currentLang === 'ru' ? '>> ВНИМАНИЕ: ВЫ ПОКИДАЕТЕ ЗАЩИЩЁННУЮ СРЕДУ HOWLING VOID.' : '>> WARNING: YOU ARE LEAVING THE HOWLING VOID SECURED ENVIRONMENT.',
        warning2: currentLang === 'ru' ? '>> ВСЕ ВНЕШНИЕ ВЗАИМОДЕЙСТВИЯ НЕ ПРОТОКОЛИРУЮТСЯ.' : '>> ALL EXTERNAL INTERACTIONS ARE NOT LOGGED.',
        confirmBtn: currentLang === 'ru' ? 'ПОДТВЕРДИТЬ ПОДКЛЮЧЕНИЕ' : 'CONFIRM CONNECTION'
    };
    ...
    discordBtn.onclick = function() {
        const connectingText = currentLang === 'ru' ? 'ПОДКЛЮЧЕНИЕ...' : 'CONNECTING...';
        const connectedText = currentLang === 'ru' ? 'ПОДКЛЮЧЕНИЕ УСТАНОВЛЕНО' : 'CONNECTION ESTABLISHED';
        const redirectingText = currentLang === 'ru' ? 'ПЕРЕНАПРАВЛЕНИЕ...' : 'REDIRECTING...';
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                window.open('https://discord.gg/qVHJ4AuW4C', '_blank', 'noopener,noreferrer');
...
function showPromises() {
    ...
    promises.forEach((promise, index) => {
        ...
        const category = promise.category ? `<div class="promise-category">[${promise.category}]</div>` : '';
...
function markPromise(id, status) {
    ...
    if (promise) {
        ...
        updateTrustDisplay(); 
        systemMessage(`${translations[currentLang].promiseTexts.status}: ${status.toUpperCase()}`);
        showPromises(); 
    }
}
...
function triggerStrongGlitch() {
    ...
    const glitchTypes = [
        { filter: 'hue-rotate(180deg) contrast(2)', transform: 'translate(8px, 4px)' },
...
function activateSystem() {
    ...
    if (screen) {
        ...
        screen.querySelector('.activation-content').style.boxShadow = '0 0 80px #6e000065';
...
function exportDossier() {
    ...
    systemMessage(currentLang === 'ru' ? '>> ЭКСПОРТ СТАТИЧНОЙ БАЗЫ ВЫПОЛНЕН' : '>> STATIC DATABASE EXPORT COMPLETE');
}
